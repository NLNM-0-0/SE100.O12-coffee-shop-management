...
import (
	...
	"backend/module/supplierdebtreportdetail/supplierdebtreportdetailmodel"
	"context"
	"errors"
	"time"
)
...
func (biz *findSupplierDebtReportBiz) FindSupplierDebtReport(
	...
	allSupplier, err := biz.supplierStore.GetAllSupplier(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, supplier := range allSupplier {
		...
		for _, supplierDebt := range supplierDebts {
			if *supplierDebt.DebtType == enum.Debt {
				debtAmount += supplierDebt.Amount
			} else if *supplierDebt.DebtType == enum.Pay {
				payAmount += supplierDebt.Amount
			}
		}
		...
		if len(supplierDebts) != 0 {
			final = supplierDebts[len(supplierDebts)-1].AmountLeft
		}
